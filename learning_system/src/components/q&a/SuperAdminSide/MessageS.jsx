...
const Message = ({ _id, message, timestamp, senderId, currentUserId, onDelete }) => {
  ...
  return (
    <div className={`relative flex ${isSentByUser ? "justify-end" : "justify-start"} my-2`} onContextMenu={handleRightClick}>
      ...
      {showOptions && isSentByUser && (
        <div className="absolute top-0 right-0 bg-white shadow-md rounded-md p-2">
          <button className="text-red-600 hover:text-red-800" onClick={() => { onDelete(_id); setShowOptions(false); }}>Delete</button>
        </div>
      )}
    </div>
  );
