...
const TeacherContent = () => {
  ...
  useEffect(() => {
    ...
    const fetchTeachingResources = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/learning/getall?audience=teacher');
  ...
  return (
    <div className="flex-1 p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold">Teaching Tips</h1>
        </div>
        ...
        {cards.length > 0 ? (
          cards.map((resource, index) => (
            <LearningCard
              ...
              image={`http://localhost:5000/api/learning/image${resource.image}`} 
            />
          ))
        ) : (
